...
export async function GET(request: Request) {
  ...
  if (brand) {
    filteredContents = filteredContents.filter(content => 
      content.brand?.toLowerCase().includes(brand.toLowerCase())
    );
  }
  ...
  switch (sort) {
    case 'newest':
      filteredContents.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      break;
    case 'oldest':
      filteredContents.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
      break;
    case 'az':
      filteredContents.sort((a, b) => a.title.localeCompare(b.title));
      break;
    case 'za':
      filteredContents.sort((a, b) => b.title.localeCompare(a.title));
      break;
    case 'rating':
      filteredContents.sort((a, b) => (b.rating || 0) - (a.rating || 0));
      break;
    default:
      break;
  }
